...
// OUTPUT_ERROR_DIALOG_STRING()は、このマクロの呼び出し先の
// "ソースファイル名" と "行番号" と "関数名" および、このマクロの"引数に定義したエラー内容の文字列" を含む
// std::string型の文字列データを出力します.。
#define OUTPUT_ERROR_STRING(errorCode) (DebugSystem::OutputErrorDialogString(errorCode, __FILE__, __LINE__, __FUNCTION__));
...
namespace DebugSystem
{
	...
	// OutputErrorDialogString()は、エラーダイアログ用の文字列を生成します。
	...
	std::string OutputErrorDialogString(std::string inErrorCode
